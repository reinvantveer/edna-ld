...
function csv2jsonSchema(filePath, collections) {
  ...
  return csvReader.csvParse(filePath, collections)
    .then(csvData => {
      ...
      return Promise.all(
        rows.map(row => collections.sourcedataCollection.updateOne(
            { _id: row._id },
            { $set: row },
            { upsert: true }
          )));
  ...
  return new Promise((resolve, reject) => {
    const relatedMap = dedupedMap
      .map(relateSchema => {
        ...
        relateSchema.closestRelatives = diffs[9] ? diffs.slice(0, 10) : [];
        return relateSchema;
      });
        ...
        relate(summarize(hashes))
          .then(summarizedHashes => {
            const withId = summarizedHashes.map(schema => {
              ...
              return xtend(schema, { _id: schema.hash });
            });
            return upsertMany(withId, collections.schemaCollection);
          })
          .then(summarizedHashes => resolve(summarizedHashes))
          .catch(err => reject(err));
      });
  });
}
